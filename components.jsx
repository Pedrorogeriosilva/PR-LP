...
function Cursor() {
  ...
  React.useEffect(() => {
    ...
    const over = (e) => {
      const t = e.target.closest('a, button, .project-card, .service-card, .process-step, .testimonial-card');
...
function Loader() {
  ...
  return (
    <div className={`loader ${hidden ? 'hidden' : ''}`}>
      <div className="loader-logo">Pedro Rogerio<span style={{color:'#c6ff00'}}>.</span></div>
...
function WhatsAppFloat() {
  return (
    <a className="wa-float" href={WA_LINK} target="_blank" rel="noreferrer" aria-label="WhatsApp">
      <span className="wa-float-icon"><WhatsAppIcon size={18} color="#c6ff00"/></span>
      Falar agora
    </a>
  );
}
...
function Reveal({ children, delay = 0, as = 'div', className = '', ...rest }) {
  ...
  React.useEffect(() => {
    ...
    if (!el) return;
    let revealed = false;
    const reveal = () => { if (!revealed) { revealed = true; el.classList.add('in'); } };
    const check = () => {
...
Object.assign(window, { Cursor, Loader, Nav, WhatsAppFloat, Reveal, Ticker, WhatsAppIcon, ArrowUR, WA_LINK });
